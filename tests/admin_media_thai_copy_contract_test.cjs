...
assert.match(entrypoint, /content-studio-v9-persist-verify/, 'Media entrypoint must cache-bust the current Thai Content Studio copy');
...
assert.match(source, /replace\(\/รูป\\s\+แบนเนอร์\/g, 'รูปแบนเนอร์'\)/, 'Promotion image label spacing must be compact Thai');
