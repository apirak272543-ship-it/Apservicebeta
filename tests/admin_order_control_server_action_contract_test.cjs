...
const entrypoint = fs.readFileSync('admin/orders.html', 'utf8');
...
assert.match(entrypoint, /control-plane-v6-thai-orders/, 'Orders entrypoint must cache-bust the Thai control-plane copy');
assert.match(entrypoint, /admin-group-payment-v3-thai-copy/, 'Orders entrypoint must cache-bust the Thai group-payment copy');
