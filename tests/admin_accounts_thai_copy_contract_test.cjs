...
const actionPatch = fs.readFileSync('admin/admin-accounts-action-copy-patch.js', 'utf8');
...
if (!actionPatch.includes('ชื่อและรหัสเข้าสู่ระบบ')) throw new Error('Deep identity action must use Thai login wording');
if (!entrypoint.includes('admin-accounts-action-copy-patch.js?v=accounts-action-v1')) throw new Error('Accounts entrypoint must load deep action patch');
