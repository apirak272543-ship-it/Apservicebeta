(() => {
  ...
  async function resetPassword(email, redirectTo) { const value = String(email || '').trim().toLowerCase(); if (!value) throw new Error('กรุณาระบุอีเมลติดต่อ'); const target = String(redirectTo || '').trim(); const path = target ? `recover?redirect_to=${encodeURIComponent(target)}` : 'recover'; return authRequest(path, { method: 'POST', body: JSON.stringify({ email: value }) }); }
