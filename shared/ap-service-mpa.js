(() => {
  ...
  async function resetPassword(email, redirectTo) { const value = String(email || '').trim().toLowerCase(); if (!value) throw new Error('กรุณาระบุอีเมลติดต่อ'); const payload = { email: value }; if (String(redirectTo || '').trim()) payload.redirect_to = redirectTo; return authRequest('recover', { method: 'POST', body: JSON.stringify(payload) }); }
  ...
  async function updatePassword(password) { if (!token()) throw new Error('ลิงก์ตั้งรหัสผ่านหมดอายุหรือไม่สมบูรณ์'); if (!password || password.length < 8) throw new Error('รหัสผ่านใหม่ต้องมีอย่างน้อย 8 ตัวอักษร'); return authRequest('user', { method: 'PUT', headers: { Authorization: `Bearer ${token()}` }, body: JSON.stringify({ password }) }); }
