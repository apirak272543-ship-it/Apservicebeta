(() => {
  ...
  function ordersPatch() {
    ...
    R.gate('orders', content).then(async access => {
      ...
      const load = async () => { host.innerHTML = M.ui.loading('กำลังโหลดข้อมูลออเดอร์…'); rows = await request('delivery_orders?select=id,customer_id,customer_name,store_id,store_name,rider_id,rider_name,ride_selected_rider_id,status,total,payable,delivery_fee,credit_used,dispatch_status,assigned_at,estimated_arrival_at,eta_source,dispatch_note,dispatch_updated_at,ordered_at,updated_at&order=ordered_at.desc&limit=500'); render(); };
      const render = () => {
        ...
        host.innerHTML = filtered.length ? `<div class="admin-order-grid">${filtered.map(row => `<article class="admin-order-card"><header><div><small>ORDER</small><h2>${esc(row.id)}</h2></div><span class="mpa-badge">${esc(statusLabel(row.status))}</span></header><dl><div><dt>ลูกค้า</dt><dd>${esc(row.customer_name || '-')}</dd></div><div><dt>ร้านค้า</dt><dd>${esc(row.store_name || '-')}</dd></div><div><dt>Rider</dt><dd>${row.rider_name ? `🛵 ${esc(row.rider_name)}` : 'ยังไม่มอบหมาย'}</dd></div><div><dt>Dispatch</dt><dd>${esc(({ unassigned: 'ยังไม่มอบหมาย', assigned: 'มอบหมายแล้ว', en_route: 'กำลังไปจุดรับ', arrived_pickup: 'ถึงจุดรับแล้ว', picked_up: 'รับสินค้าแล้ว', delivering: 'กำลังไปส่ง', delivered: 'ส่งสำเร็จ', exception: 'มีเหตุขัดข้อง' }[row.dispatch_status] || (row.rider_id ? 'มอบหมายแล้ว' : 'ยังไม่มอบหมาย')))}</dd></div><div><dt>ETA</dt><dd>${row.estimated_arrival_at ? esc(new Date(row.estimated_arrival_at).toLocaleString('th-TH')) : 'ยังไม่กำหนด'}</dd></div><div><dt>เวลา</dt><dd>${row.ordered_at ? esc(new Date(row.ordered_at).toLocaleString('th-TH')) : '-'}</dd></div></dl><div class="admin-order-summary"><span>ยอดชำระ <b>${money(row.payable ?? row.total)}</b></span><span>ยอดรวม ${money(row.total)}</span></div><div class="admin-order-actions"><button class="mpa-button" data-status-order="${esc(row.id)}">เปลี่ยนสถานะ</button><button class="mpa-button mpa-button-secondary" data-edit-order="${esc(row.id)}">แก้รายการ</button><button class="mpa-button mpa-button-secondary" data-assign-order="${esc(row.id)}">เลือก Rider</button><button class="mpa-button mpa-button-secondary" data-dispatch-order="${esc(row.id)}">Dispatch / ETA</button><button class="mpa-button mpa-button-secondary" data-history-order="${esc(row.id)}">ประวัติ</button></div></article>`).join('')}</div>` : `<div class="mpa-state"><p>${emptyMessages[tab]}</p></div>`;
