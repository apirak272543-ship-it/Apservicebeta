(() => {
  ...
  function dashboardPatch() {
    ...
    R.gate('dashboard', content).then(async access => {
      ...
      const [orders, stores, riders, withdrawals] = await Promise.all([
  ...
  function mediaCenterPatch() {
    ...
    R.gate('media', content).then(async access => {
      ...
      const load = async () => {
        ...
        brandHost.innerHTML = `<h2 style="margin-top:0">แบรนด์หน้า Customer</h2><form id="brandForm"><div class="admin-form-grid"><label class="mpa-field"><span>Logo URL</span><input name="logo_url" value="${esc(fields.logo)}" placeholder="https://…"></label><label class="mpa-field"><span>Background URL</span><input name="background_url" value="${esc(fields.background)}" placeholder="https://…"></label><label class="mpa-field admin-form-full"><span>Banner URL (หนึ่ง URL ต่อบรรทัด)</span><textarea name="banner_urls" rows="3" placeholder="https://…">${esc(fields.banners.join('\n'))}</textarea></label></div><div class="admin-media-pair"><div>${fields.logo ? `<img src="${esc(fields.logo)}" alt="Logo ปัจจุบัน">` : '<p class="mpa-muted">ยังไม่มี Logo</p>'}</div><div>${fields.background ? `<img src="${esc(fields.background)}" alt="Background ปัจจุบัน">` : '<p class="mpa-muted">ยังไม่มี Background</p>'}</div></div><button class="mpa-button" type="submit">บันทึกแบรนด์และ Banner</button></form>`;
        brandHost.querySelector('#brandForm').onsubmit = async event => {
