(() => {
  ...
  function ordersPatch() {
    ...
    R.gate('orders', content).then(async access => {
      ...
      const workState = row => { const bucket = orderBucket(row.status); if (bucket === 'history') return { key: 'done', label: 'จัดการแล้ว', icon: '✓' }; if (bucket === 'new') return { key: 'attention', label: 'ยังไม่ได้จัดการ', icon: '!' }; if (row.dispatch_status === 'delivered') return { key: 'attention', label: 'รอปิดงาน', icon: '!' }; return { key: 'progress', label: 'กำลังดำเนินการ', icon: '…' }; };
      ...
      const openOrderDetail = row => {
        const detail = modal(`รายละเอียดออเดอร์ · ${row.id}`, `<div class="admin-order-detail-summary"><span class="mpa-badge">${esc(statusText(row))}</span><p class="mpa-muted">สั่งเมื่อ ${row.ordered_at ? esc(new Date(row.ordered_at).toLocaleString('th-TH')) : '-'}</p></div><dl class="admin-order-detail-grid"><div><dt>เลขออเดอร์</dt><dd>${esc(row.id)}</dd></div><div><dt>ลูกค้า</dt><dd>${esc(row.customer_name || '-')}</dd></div><div><dt>ร้านค้า</dt><dd>${esc(row.store_name || '-')}</dd></div><div><dt>Rider</dt><dd>${row.rider_name ? `🛵 ${esc(row.rider_name)}` : 'ยังไม่มอบหมาย'}</dd></div><div><dt>การจัดส่ง</dt><dd>${esc(dispatchLabel(row))}</dd></div><div><dt>วิธีชำระ</dt><dd>${esc(row.payment_method || 'ไม่ระบุ')}</dd></div><div><dt>ยอดสินค้า</dt><dd>${money(row.total)}</dd></div><div><dt>ค่าจัดส่ง</dt><dd>${money(row.delivery_fee)}</dd></div><div><dt>ยอดชำระ</dt><dd><strong>${money(row.payable ?? row.total)}</strong></dd></div><div><dt>ETA</dt><dd>${row.estimated_arrival_at ? esc(new Date(row.estimated_arrival_at).toLocaleString('th-TH')) : 'ยังไม่กำหนด'}</dd></div></dl><div class="admin-order-detail-actions"><button class="mpa-button" data-detail-status>เปลี่ยนสถานะ</button><button class="mpa-button mpa-button-secondary" data-detail-edit>แก้รายการ</button><button class="mpa-button mpa-button-secondary" data-detail-assign>เลือก Rider</button><button class="mpa-button mpa-button-secondary" data-detail-dispatch>Dispatch / ETA</button><button class="mpa-button mpa-button-secondary" data-detail-history>ประวัติออเดอร์</button></div>`, `รายละเอียดออเดอร์ ${row.id}`);
