(() => {
  ...
  function ordersPatch() {
    const R = runtime();
    if (!R) return;
    const content = `<div class="mpa-page-head"><div><p class="admin-page-eyebrow">ORDER CONTROL</p><h1>จัดการออเดอร์</h1><p>แยกคิวงานตามขั้นตอนจริง เพื่อให้รู้ทันทีว่าออเดอร์ไหนต้องรับเรื่อง ออเดอร์ไหนกำลังทำ และออเดอร์ไหนปิดงานแล้ว</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="mpa-button mpa-button-secondary" id="showHistory">ดูประวัติออเดอร์</button><button class="mpa-button mpa-button-secondary" id="refreshOrders">รีเฟรช</button></div></div><section class="mpa-card admin-orders-workspace"><div class="admin-filter-row admin-orders-tabs" role="tablist" aria-label="คิวออเดอร์"><button class="mpa-button" data-order-tab="new" role="tab" aria-selected="true">ต้องรับเรื่อง <span data-order-count="new">0</span></button><button class="mpa-button mpa-button-secondary" data-order-tab="active" role="tab" aria-selected="false">กำลังดำเนินการ <span data-order-count="active">0</span></button><button class="mpa-button mpa-button-secondary" data-order-tab="history" role="tab" aria-selected="false">ปิดงานแล้ว <span data-order-count="history">0</span></button><label class="mpa-field admin-order-search"><span class="sr-only">ค้นหาออเดอร์</span><input id="orderSearch" placeholder="ค้นหาเลขออเดอร์ ลูกค้า ร้านค้า หรือ Rider"></label></div><div id="orderStatusFilters" class="admin-order-status-filters" aria-label="กรองตามสถานะ"></div><div id="orders">${M.ui.loading('กำลังโหลดออเดอร์…')}</div></section>`;
    R.gate('orders', content).then(async access => {
      if (!access) return;
      R.user = access.user;
      const host = document.querySelector('#orders'); let rows = []; let tab = 'new'; let search = ''; let statusFilter = 'all';
      ...
      const renderCard = row => `<article class="admin-order-card"><header><div><small>ORDER</small><h2>${esc(row.id)}</h2><time datetime="${esc(row.ordered_at || '')}">${row.ordered_at ? esc(new Date(row.ordered_at).toLocaleString('th-TH')) : '-'}</time></div><span class="mpa-badge">${esc(statusText(row))}</span></header><dl><div><dt>ลูกค้า</dt><dd>${esc(row.customer_name || '-')}</dd></div><div><dt>ร้านค้า</dt><dd>${esc(row.store_name || '-')}</dd></div><div><dt>Rider</dt><dd>${row.rider_name ? `🛵 ${esc(row.rider_name)}` : 'ยังไม่มอบหมาย'}</dd></div><div><dt>การจัดส่ง</dt><dd>${esc(dispatchLabel(row))}</dd></div><div><dt>วิธีชำระ</dt><dd>${esc(row.payment_method || 'ไม่ระบุ')}</dd></div></dl><div class="admin-order-summary"><span>ยอดชำระ <b>${money(row.payable ?? row.total)}</b></span><span>ค่าจัดส่ง ${money(row.delivery_fee)}</span></div><div class="admin-order-actions"><button class="mpa-button" data-status-order="${esc(row.id)}">เปลี่ยนสถานะ</button><button class="mpa-button mpa-button-secondary" data-edit-order="${esc(row.id)}">แก้รายการ</button><button class="mpa-button mpa-button-secondary" data-assign-order="${esc(row.id)}">เลือก Rider</button><button class="mpa-button mpa-button-secondary" data-dispatch-order="${esc(row.id)}">Dispatch / ETA</button><button class="mpa-button mpa-button-secondary" data-history-order="${esc(row.id)}">ประวัติ</button></div></article>`;
      const bindCards = filtered => filtered.forEach(row => { document.querySelector(`[data-status-order="${CSS.escape(row.id)}"]`)?.addEventListener('click', () => orderStatusEditor(row, load)); document.querySelector(`[data-edit-order="${CSS.escape(row.id)}"]`)?.addEventListener('click', () => orderItemEditor(row, load)); document.querySelector(`[data-assign-order="${CSS.escape(row.id)}"]`)?.addEventListener('click', () => riderAssignment(row, load)); document.querySelector(`[data-dispatch-order="${CSS.escape(row.id)}"]`)?.addEventListener('click', () => dispatchEditor(row, load)); document.querySelector(`[data-history-order="${CSS.escape(row.id)}"]`)?.addEventListener('click', () => orderHistory(row)); });
      const render = () => {
        ...
        const inTab = (rows || []).filter(row => orderBucket(row.status) === tab);
        ...
        const filtered = inTab.filter(row => (statusFilter === 'all' || statusText(row) === statusFilter) && (!search || [row.id, row.customer_name, row.store_name, row.rider_name].some(value => String(value || '').toLowerCase().includes(search.toLowerCase()))));
        const emptyMessages = { new: 'ยังไม่มีออเดอร์ที่ต้องรับเรื่อง', active: 'ไม่มีออเดอร์ที่กำลังดำเนินการ', history: 'ยังไม่มีออเดอร์ที่ปิดงานแล้ว' };
