(() => {
  ...
  async function review(payment) {
    ...
    dialog.node.querySelector('#saveGroupPaymentReview').onclick = async () => {
      ...
      try {
        const governance = await window.APServiceAdminOverride.collect(dialog.node, 'group-payment', `คุณกำลัง${decision === 'verify' ? 'ยืนยัน' : 'ปฏิเสธ'}การชำระเงินของกลุ่ม ${group.id}`);
        await M.request('rpc/admin_review_checkout_group_payment', { method: 'POST', private: true, body: JSON.stringify({ p_checkout_group_id: group.id, p_decision: decision, p_reason: governance.reason, p_idempotency_key: `admin-group-payment-${crypto.randomUUID()}`, p_evidence_path: governance.evidencePath || null }) });
  ...
  async function enhance() {
    ...
    if (!host || enhancing) return;
    enhancing = true;
    let payments;
    try {
      payments = await loadPending();
      let queue = document.querySelector('#checkoutGroupPaymentQueue');
      if (!queue) {
      ...
      // ไม่สร้างหรือเปลี่ยนแปลง UI หากคิวการชำระเงินชั่วคราวโหลดไม่สำเร็จ
    } finally {
      enhancing = false;
    }
  }
  ...
  const scheduleEnhance = () => {
    if (scheduled || enhancing || document.querySelector('#checkoutGroupPaymentQueue') || !document.querySelector('#orders')) return;
    scheduled = true;
