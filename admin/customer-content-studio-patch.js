  ...
  function contentStudio() {
    ...
    R.gate('media', content).then(async access => {
      ...
      const load = async () => {
        ...
        assets = assetRows || [];
        host.innerHTML = `<form id="customerContentForm"><section class="mpa-card"><h2 style="margin-top:0">Hero หน้าแรก · พื้นหลังและข้อความหลัก</h2><p class="mpa-muted">ส่วนนี้รองรับรูปพื้นหลังจากรูปที่ 3 และแก้ปุ่มทั้งหมดโดยไม่เปลี่ยน route guard หรือ business logic</p><div class="admin-form-grid">${field('Eyebrow', 'hero.eyebrow', home.hero.eyebrow)}${field('หัวข้อหลัก', 'hero.title', home.hero.title)}${area('คำอธิบาย', 'hero.description', home.hero.description, 3)}${field('Overlay', 'hero.overlay', home.hero.overlay)}${field('สีข้อความ', 'hero.textColor', home.hero.textColor)}${field('Primary label', 'hero.primaryAction.label', home.hero.primaryAction.label)}${field('Primary href', 'hero.primaryAction.href', home.hero.primaryAction.href)}${toggle('เปิด Primary', 'hero.primaryAction.enabled', home.hero.primaryAction.enabled)}${field('Secondary label', 'hero.secondaryAction.label', home.hero.secondaryAction.label)}${field('Secondary href', 'hero.secondaryAction.href', home.hero.secondaryAction.href)}${toggle('เปิด Secondary', 'hero.secondaryAction.enabled', home.hero.secondaryAction.enabled)}${mediaInput('Hero background (รูปที่ 3)', 'hero.backgroundUrl', home.hero.backgroundUrl, 'ADMIN_MEDIA', 'ภาพพื้นหลังหน้า Customer')}${mediaInput('Hero art/ภาพประกอบ', 'hero.artUrl', home.hero.artUrl, 'ADMIN_MEDIA', 'ภาพประกอบ Hero')}</div></section><section class="mpa-card" style="margin-top:16px"><h2 style="margin-top:0">ส่วนเลือกบริการ</h2><div class="admin-form-grid">${field('Eyebrow', 'serviceSection.eyebrow', home.serviceSection.eyebrow)}${field('หัวข้อ', 'serviceSection.title', home.serviceSection.title)}${area('คำอธิบาย', 'serviceSection.description', home.serviceSection.description, 2)}${toggle('แสดงส่วนบริการ', 'serviceSection.enabled', home.serviceSection.enabled)}</div>${home.serviceCards.slice().sort((a,b) => a.sortOrder - b.sortOrder).map(renderCard).join('')}</section><section class="mpa-card" style="margin-top:16px"><h2 style="margin-top:0">ร้านค้ายอดนิยมและตะกร้า</h2><div class="admin-form-grid">${field('หัวข้อร้านค้า', 'storeSection.title', home.storeSection.title)}${field('คำอธิบายร้านค้า', 'storeSection.description', home.storeSection.description)}${field('ปุ่มดูทั้งหมด', 'storeSection.viewAllLabel', home.storeSection.viewAllLabel)}${field('ลิงก์ดูทั้งหมด', 'storeSection.viewAllHref', home.storeSection.viewAllHref)}${toggle('แสดงร้านค้ายอดนิยม', 'storeSection.enabled', home.storeSection.enabled)}${field('ไอคอนตะกร้า', 'floatingCart.icon', home.floatingCart.icon)}${field('ข้อความตะกร้า', 'floatingCart.label', home.floatingCart.label)}${field('ลิงก์ตะกร้า', 'floatingCart.href', home.floatingCart.href)}${toggle('แสดงตะกร้า', 'floatingCart.enabled', home.floatingCart.enabled)}</div></section><section class="mpa-card" style="margin-top:16px"><h2 style="margin-top:0">ข้อความบน Header</h2><div class="admin-form-grid">${field('สมัครสมาชิก', 'navigation.registerLabel', home.navigation.registerLabel)}${field('ช่วยเหลือ', 'navigation.supportLabel', home.navigation.supportLabel)}${field('การแจ้งเตือน', 'navigation.notificationLabel', home.navigation.notificationLabel)}${field('โปรไฟล์', 'navigation.profileLabel', home.navigation.profileLabel)}</div></section>${promotions.length ? `<section style="margin-top:16px"><h2>Banner โฆษณา · แก้ไขละเอียดทุกองค์ประกอบ</h2>${promotions.map(renderPromotion).join('')}</section>` : `<section class="mpa-card" style="margin-top:16px"><h2 style="margin-top:0">Banner โฆษณา</h2><p class="mpa-muted">ยังไม่มีรายการใน customer_promotions ให้เพิ่มผ่านระบบ promotions เดิมก่อน แล้วกลับมาแก้ไขรายละเอียดที่นี่</p></section>`}<section class="mpa-card" style="margin-top:16px"><h2 style="margin-top:0">Media registry ที่ตรวจพบ</h2><p class="mpa-muted">รายการนี้เป็นหลักฐานจาก media_assets จริง ไม่ได้หมายความว่าจะโหลดทุกไฟล์มาใช้ในหน้า Customer</p><p><b>${assets.length}</b> รายการล่าสุดที่ registry อ่านได้</p><div class="mpa-table-wrap"><table class="mpa-table"><thead><tr><th>ประเภท</th><th>สถานะ</th><th>ขนาด</th><th>สร้างเมื่อ</th></tr></thead><tbody>${assets.slice(0, 20).map(row => `<tr><td>${esc(row.media_type || '-')}</td><td>${esc(row.status || '-')}</td><td>${esc(row.byte_size ? `${Math.ceil(row.byte_size / 1024)} KB` : '-')}</td><td>${esc(row.created_at ? new Date(row.created_at).toLocaleString('th-TH') : '-')}</td></tr>`).join('') || '<tr><td colspan="4">ยังไม่มี media registry</td></tr>'}</tbody></table></div></section><div class="admin-modal-actions" style="margin-top:16px"><button class="mpa-button" type="submit">บันทึก Content และ Banner ทั้งหมด</button></div></form>`;
