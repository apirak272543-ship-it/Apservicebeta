(() => {
  ...
  const defaultCards = [
    { id: 'food', enabled: true, sortOrder: 10, icon: '🍜', title: 'สั่งอาหาร', description: 'เลือกจากร้านเด็ดใกล้ตัว พร้อมเมนูยอดนิยม', href: 'stores.html', actionLabel: 'เปิดบริการ', altText: 'สั่งอาหาร', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' },
    { id: 'supermarket', enabled: true, sortOrder: 20, icon: '🛒', title: 'ซูเปอร์มาร์เก็ต', description: 'เลือกซื้อจากร้านใกล้คุณในหน้าเดียว', href: 'stores.html', actionLabel: 'เลือกซื้อสินค้า', altText: 'ซูเปอร์มาร์เก็ต', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' },
    { id: 'parcel', enabled: true, sortOrder: 30, icon: '🛵', title: 'ส่งของ A → B', description: 'เรียกไรเดอร์รับของจากจุดหนึ่งไปส่งยังอีกจุดหนึ่ง', href: 'parcel.html', actionLabel: 'สร้างรายการส่งของ', altText: 'ส่งของจากจุด A ไปจุด B', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' },
    { id: 'orders', enabled: true, sortOrder: 40, icon: '🧺', title: 'ออเดอร์ของฉัน', description: 'ติดตามสถานะและดูประวัติการใช้บริการ', href: 'orders.html', actionLabel: 'ดูออเดอร์', altText: 'ออเดอร์ของฉัน', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' },
    { id: 'marketplace', enabled: true, sortOrder: 50, icon: '🛍️', title: 'ตลาดชุมชน', description: 'เลือกซื้อ ขาย และแชตกับสมาชิก AP Service', href: 'marketplace.html', actionLabel: 'เข้าสู่ตลาดชุมชน', altText: 'ตลาดชุมชน', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' },
  ];
  ...
  const normalizeHome = value => {
    ...
    const rawCards = Array.isArray(source.serviceCards) ? source.serviceCards : []; const byId = new Map(rawCards.map(row => [String(row?.id || ''), row])); const extras = rawCards.filter(row => row && row.id && !defaultCards.some(card => card.id === String(row.id)));
    return {
      ...defaultHome,
      ...source,
      hero,
      serviceSection: { ...defaultHome.serviceSection, ...safeJson(source.serviceSection) },
      serviceCards: [...defaultCards.map(card => mergeCard(card, byId.get(card.id))), ...extras.map(row => mergeCard({ id: String(row.id), enabled: true, sortOrder: 100, icon: '✦', title: String(row.title || row.id), description: '', href: 'index.html', actionLabel: 'ดูรายละเอียด', altText: String(row.title || row.id), textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' }, row))],
  ...
  const routeField = (label, name, value, helpText = 'ใส่ลิงก์ภายในเว็บ เช่น parcel.html หรือ URL ภายนอกที่ขึ้นต้นด้วย https:// ได้') => `<label class="mpa-field"><span>${esc(label)}${help(helpText)}</span><input name="${esc(name)}" data-content-field="${esc(name)}" type="url" value="${esc(value)}" placeholder="https://example.com หรือ page.html"></label>`;
  const targetField = (label, name, value) => `<label class="mpa-field"><span>${esc(label)}</span><select name="${esc(name)}" data-content-field="${esc(name)}"><option value="_self" ${value !== '_blank' ? 'selected' : ''}>เปิดหน้าเดิม</option><option value="_blank" ${value === '_blank' ? 'selected' : ''}>เปิดแท็บใหม่</option></select></label>`;
  ...
  function readForm(form, home, promotions) {
    ...
    next.serviceCards = next.serviceCards.map(card => ({ ...card, enabled: bool(`card.${card.id}.enabled`), sortOrder: clamp(value(`card.${card.id}.sortOrder`), 0, 999, card.sortOrder), icon: value(`card.${card.id}.icon`), iconUrl: value(`card.${card.id}.iconUrl`), backgroundUrl: value(`card.${card.id}.backgroundUrl`), title: value(`card.${card.id}.title`), description: value(`card.${card.id}.description`), href: safeHref(value(`card.${card.id}.href`)) || card.href, target: ['_self','_blank'].includes(value(`card.${card.id}.target`)) ? value(`card.${card.id}.target`) : (card.target || '_self'), actionLabel: value(`card.${card.id}.actionLabel`), altText: value(`card.${card.id}.altText`), textColor: safeColor(value(`card.${card.id}.textColor`), '#102a2a'), backgroundColor: safeColor(value(`card.${card.id}.backgroundColor`), '#ffffff') }));
  ...
  function contentStudio() {
    ...
    R.gate('media', content).then(async access => {
      ...
      const load = async () => {
        ...
        host.querySelectorAll('[data-content-save]').forEach(button => button.onclick = () => saveSection(button.dataset.contentSave, button)); host.querySelector('[data-card-action="add"]')?.addEventListener('click', () => { const id = `custom-${Date.now()}`; const card = { id, enabled: true, sortOrder: Math.max(0, ...home.serviceCards.map(item => Number(item.sortOrder) || 0)) + 10, icon: '✦', title: 'การ์ดใหม่', description: 'รายละเอียดบริการหรือโฆษณา', href: 'index.html', actionLabel: 'ดูรายละเอียด', altText: 'การ์ดใหม่', textColor: '#102a2a', backgroundColor: '#ffffff', target: '_self' }; home.serviceCards.push(card); const panel = host.querySelector('[data-content-panel="services"]'); const storeFields = panel?.querySelector('.admin-form-grid:last-of-type'); storeFields?.insertAdjacentHTML('beforebegin', renderCard(card)); panel?.querySelector(`[name="card.${CSS.escape(id)}.title"]`)?.focus(); notice('เพิ่มการ์ดใหม่แล้ว กรุณาตั้งค่าภาพและปลายทาง จากนั้นกดบันทึกบริการและร้านค้า'); });
